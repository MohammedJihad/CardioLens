...
shows the evaluation *honestly* — what the model gets right, where it transfers, and
where its confidence quietly breaks down.

> ⚠️ **Not medical advice.** Every figure is a *model score on historical public
> research data* — not a medical verdict and not a clinical tool. Trained on a small,
> decades-old, single-centre dataset with **no clinical validation**.

<!-- After deploying (see docs/DEPLOYMENT.md), fill these in: -->
**🔗 Live demo:** `https://<your-site>.vercel.app` · **API:** `https://<your-api>.onrender.com/docs`
**📄 Project dossier (36-page PDF):** [`docs/final-dossier/CardioLens_Project_Dossier.pdf`](docs/final-dossier/CardioLens_Project_Dossier.pdf)
...
1. **The science** — a leakage-free pipeline that cleans the data, fixes a known
   ...
   cohorts it never trained on.** This is **frozen** — the website never recomputes it.
2. **The website (CardioLens)** — a Next.js + FastAPI experience that presents those
...
**Model selection — stratified 5-fold CV (training split):**
...
The top three are a statistical tie — on data this small, a simple interpretable
model is as defensible as the ensemble.

**Held-out test (61 cases, threshold 0.50):** ROC-AUC **0.892**, PR-AUC 0.853,
sensitivity 0.714, specificity 0.848, F1 0.755, Brier 0.137. Confusion: TP 20 ·
FN 8 (missed positive-label cases) · TN 28 · FP 5. Intervals are wide (e.g. ROC-AUC
**0.892 [0.805, 0.961]**) because the test set is small.

**The central lesson — external validation:** the model (trained only on Cleveland)
...
|---|---:|---:|---:|---:|
| Cleveland (internal) | 61 | 0.892 | 0.137 | — |
| Hungarian | 294 | 0.857 | 0.178 | 99% / 90% |
| VA Long Beach | 200 | 0.672 | 0.339 | 99% / 83% |
| Switzerland | 123 | 0.810 | 0.337 | 96% / 42% |

The model's strongest features (`ca`, `thal`) are 42–99% missing externally, so its
probabilities stop meaning what they say off the training cohort.

## The website

Seven pages: **/** (thesis + honest stats) · **/try** (live model score + explanation)
· **/results** (internal metrics) · **/external** (the stress test) · **/thresholds**
(operating-point trade-off) · **/transparency** (full model + data cards) · **/about**
...
- **/try** sends one input pattern to the API, returns a **model score** + a gauge +
  ...
  database, no analytics of inputs — scored and forgotten.
...
```
heart-disease-ml/
├── src/               data · preprocess · train · evaluate · explain · predict   (FROZEN)
├── models/            best_model.joblib — calibrated Random Forest               (FROZEN)
├── reports/           metrics.json · model_card.md · external validation · …     (FROZEN)
├── data/              raw/ (committed) + processed/ (regenerated)                 (FROZEN)
├── notebooks/ tests/  original course notebook · pytest suite                    (FROZEN)
├── api/               main.py — FastAPI service (+ requirements.txt)
├── frontend/          Next.js 14 site (App Router, TS, Tailwind)
├── web-data/          reports-summary.json (snapshot of the frozen reports)
├── docs/              design/ · WEBSITE_PLAN.md · DEPLOYMENT.md · final-dossier/
├── Dockerfile · render.yaml   deploy configs for the API
└── README.md · LICENSE
```
...
GitHub → backend on **Render** (or Hugging Face Spaces) → frontend on **Vercel** →
...
DOI 10.24432/C52P4X, CC BY 4.0 — Janosi, Steinbrunn, Pfisterer, Detrano (1988).
